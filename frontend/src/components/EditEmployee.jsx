...
function EditEmployee({ employeeId }) {
  ...
  useEffect(() => {
    const fetchEmployeeData = async () => {
      try {
        ...
        const { name, email, mobileNo, designation, gender, course } = res.data;
        setFormData({ name, email, mobileNo, designation, gender, course, image: null });
      } catch (err) {
        console.error(err);
        setErrors({ form: 'Failed to load employee data.' });
      }
    };
    fetchEmployeeData();
  }, [employeeId]);
  ...
  return (
    <div className="create-employee-container">
      <h3 className="form-title">Edit Employee</h3>
